...
import type { WorkShift } from './index';
...
export class WorkShiftModel {
  ...
  /**
   * Check if this shift conflicts with another on the same day
   */
  conflictsWith(other: WorkShiftModel): boolean {
    // Must be on the same date
    if (this.shiftDate.toDateString() !== other.shiftDate.toDateString()) {
      return false;
    }

    // Full day conflicts with any other shift type
    if (this.isFullDay() || other.isFullDay()) {
      return this.shiftType !== other.shiftType;
    }
